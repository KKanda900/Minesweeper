...
'''
Advanced_Agent_EC Class
Description: A class used for the extra credit implemented on the extra credit.
'''
...
class Advanced_Agent_EC:
    ...
    mine_tracker = 0 # keeps track of the amount of mines found on the board in total
    ...
    # take the dim*dim board and start to randomly place mines and the hints around the board
    ...
    def nsfm(self, x, y):
        ...
        while (self.visited != self.dim * self.dim) and self.mine_tracker != self.mines:  # keep going until all the cells are visited
            ...
            if value == 'm':  # if it's a mine we update the knowledge base accordingly
                ...
                if len(numbers) >= 3:  # go through the queue that has cells with clue value > 0
                    ...
                    tempnum.remove(tempnum[0])
                    tempnum.append(first)
                    self.solver_3(tempnum[0], tempnum[1], tempnum[2])  # solve with the last 2 and the first cell
                elif len(numbers) == 2:  # if the queue only has 2 value we use the solver_2
                    one = numbers.popleft()
                    tempnum.append(one)
                    two = numbers.popleft()
                    tempnum.append(two)
                    self.solver_2(self.build_eq(tempnum[0]), self.build_eq(tempnum[1]))
            ...
            while bln:
                ...
                for i in range(0, self.dim):
                    ...
                    if len(numbers) >= 3:  # go through the queue that has cells with clue value > 0
                        ...
                        tempnum.remove(tempnum[0])  # solve with the last 2 and the first cell
                        tempnum.append(first)
                        self.solver_3(tempnum[0], tempnum[1], tempnum[2])  # solve with the last 2 and the first cell
                    elif len(numbers) == 2:  # if the queue only has 2 value we use the solver_2
                        one = numbers.popleft()
        ...
        print("Advanced Agent (EC) Results: {} triggered mines, {} discovered mines {} safe cells".format(c, (self.mines-c), ((self.dim*self.dim)-self.mines)))
    ...
    def start(self):
        ...
        # select a random point to start from
        random_point = random.choice(self.possible_coordinates)
        x = random_point[0]
        y = random_point[1]
...
# to start the advanced agent with ec
def start_ec_agent(dim, density):
    board = Advanced_Agent_EC(dim, density)
    board.start()
